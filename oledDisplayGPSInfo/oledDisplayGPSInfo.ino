...
void loop()
{
  ...
  if( GPS.location.age() < 1000 ) {
    ...
    display.drawString(120, 0, "X");
  }

  index = sprintf(str,"lat : %d.%d     lon : %d.%d", (int)GPS.location.lat(), fracPart(GPS.location.lat(),4), (int)GPS.location.lng(), fracPart(GPS.location.lng(),4));
  str[index] = 0;
  display.drawString(0, 16, str);   
  
  index = sprintf(str,"alt : %d.%d m    spd : %d.%d kph", (int)GPS.altitude.meters(), fracPart(GPS.altitude.meters(),2), (int)GPS.speed.kmph(), fracPart(GPS.speed.kmph(),3));
  str[index] = 0;
  display.drawString(0, 32, str);
   
  index = sprintf(str,"hdop : %d.%d   sats : %d", (int)GPS.hdop.hdop(), fracPart(GPS.hdop.hdop(),2), (int)GPS.satellites.value());
  str[index] = 0;
