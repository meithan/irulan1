...
#define SEND_DELAY     200    // milliseconds between sends
...
void loop()
{
	delay(SEND_DELAY);
	txNumber += 1;
	sprintf(txpacket, "%s %i", "Packet number", txNumber);  //start a package
//	sprintf(txpacket+strlen(txpacket),"%d",txNumber); //add to the end of package
	
	turnOnRGB(0x00ff00,0); //change rgb color
	Serial.printf("\r\nsending packet %i , length %d\r\n", txNumber, strlen(txpacket));
